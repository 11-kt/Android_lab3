...
class NavigationTest {
    ...
    @Test
    fun testRecreate() {
        val act1 = launchActivity<MainActivity>()
        ...
        onView(withId(listOfFrag[0])).check(matches(isDisplayed()))
        onView(withId(listOfFrag[0])).check(matches(hasChildCount(2)))
        onView(withId(listOfBut[0])).check(matches(isDisplayed()))
        var about = launchActivity<AboutActivity>()
        about.onActivity { activity ->
        ...
        // Проверка About
        val act2 = launchActivity<SecondActivity>()
        act2.recreate()
        onView(withId(listOfFrag[1])).check(matches(isDisplayed()))
        onView(withId(listOfFrag[1])).check(matches(hasChildCount(3)))
        onView(withId(listOfBut[1])).check(matches(isDisplayed()))
        onView(withId(listOfBut[2])).check(matches(isDisplayed()))
        about = launchActivity()
        about.onActivity { activity ->
            activity.requestedOrientation = SCREEN_ORIENTATION_LANDSCAPE
        }
        sleep(1000)
        ...
        val act3 = launchActivity<ThirdActivity>()
        act3.recreate()
        onView(withId(listOfFrag[2])).check(matches(isDisplayed()))
        onView(withId(listOfFrag[2])).check(matches(hasChildCount(3)))
